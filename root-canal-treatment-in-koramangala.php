<?php include 'navbar.php'; ?>

<main>
    <!-- Existing Invisalign class names are intentionally reused so every
         treatment page follows the same approved design and responsive CSS. -->
    <section class="invisalign_first_section">
        <div class="container-fluid invisalign_first_section_container">
            <div class="row g-0 invisalign_first_section_row">
                <div class="col-lg-5 invisalign_first_section_left">
                    <div class="invisalign_first_section_curve"></div>

                    <div class="invisalign_first_section_content">
                        <nav class="invisalign_first_section_breadcrumb" aria-label="Breadcrumb">
                            <a href="home.php">Home</a>
                            <i class="bi bi-chevron-right" aria-hidden="true"></i>
                            <a href="treatments.php">Treatments</a>
                            <i class="bi bi-chevron-right" aria-hidden="true"></i>
                            <span>Root Canal</span>
                        </nav>

                        <div class="invisalign_first_section_eyebrow">
                            ROOT CANAL TREATMENT                        </div>

                        <h1 class="invisalign_first_section_title">
                            Treat Infection,<br>Preserve Your Tooth                        </h1>

                        <p class="invisalign_first_section_description">
                            Root canal treatment removes infected or inflamed tissue from inside a tooth, then cleans, fills and seals it.                        </p>

                        <div class="invisalign_first_section_benefits">
                                                            
                                <div class="invisalign_first_section_benefit">
                                    <div class="invisalign_first_section_benefit_icon">
                                        <!-- <i class="bi bi-heart-pulse" aria-hidden="true"></i> -->
                        <img src="./assets/img/icons/root-canal.png" alt="" style="height:60px; width:60px">
                                    </div>
                                    <div class="invisalign_first_section_benefit_text">
                                        <strong>Targets</strong>
                                        <span>Infection</span>
                                    </div>
                                </div>
                                                                                                <div class="invisalign_first_section_benefit_divider"></div>
                                
                                <div class="invisalign_first_section_benefit">
                                    <div class="invisalign_first_section_benefit_icon">
                                        <!-- <i class="bi bi-shield-plus" aria-hidden="true"></i> -->
                        <img src="./assets/img/icons/Healthy-teeth (1).png" alt="" style="height:60px; width:60px">
                                    </div>
                                    <div class="invisalign_first_section_benefit_text">
                                        <strong>Helps Save</strong>
                                        <span>Natural Teeth</span>
                                    </div>
                                </div>
                                                                                                <div class="invisalign_first_section_benefit_divider"></div>
                                
                                <div class="invisalign_first_section_benefit">
                                    <div class="invisalign_first_section_benefit_icon">
                                        <!-- <i class="bi bi-lock" aria-hidden="true"></i> -->
                        <img src="./assets/img/icons/root-canal-step-4.png" alt="" style="height:60px; width:60px">
                                    </div>
                                    <div class="invisalign_first_section_benefit_text">
                                        <strong>Cleaned</strong>
                                        <span>&amp; Sealed</span>
                                    </div>
                                </div>
                                                    </div>

                        <div class="invisalign_first_section_actions">
                            <a href="contact.php" class="invisalign_first_section_book">
                                <span>Book a Consultation</span>
                                <i class="bi bi-arrow-right" aria-hidden="true"></i>
                            </a>

                            <!-- <a href="contact.php" class="invisalign_first_section_video_btn">
                                <span class="invisalign_first_section_play">
                                    <i class="bi bi-chat-dots" aria-hidden="true"></i>
                                </span>
                                <span class="invisalign_first_section_video_text">
                                    <strong>Ask a Question</strong>
                                    <small>Talk to us about tooth pain</small>
                                </span>
                            </a> -->
                        </div>
                    </div>
                </div>

                <div class="col-lg-7 invisalign_first_section_right">
                    <div class="invisalign_first_section_image_wrapper">
                        <img src="assets/img/treatments/root-canal-rec-1.png"
                             alt="Root canal treatment for an infected tooth"
                             class="invisalign_first_section_image">

                        <div class="invisalign_first_section_handwriting_one" aria-hidden="true">
                            <span>Relieve</span>
                            <span>Treat</span>
                            <span>Protect</span>
                            <div>
                                <span>Restore</span>
                                <i class="bi bi-heart"></i>
                            </div>
                        </div>

                        <div class="invisalign_first_section_handwriting_two" aria-hidden="true">
                            <span>Keep</span>
                            <span>Your</span>
                            <span>Natural Tooth</span>
                            <i class="bi bi-heart"></i>
                        </div>

                        <div class="invisalign_first_section_quote_card" aria-hidden="true">
                                                            <span>Comfort</span>
                                                            <span>Begins</span>
                                                            <span>With Care</span>
                                                        <div class="invisalign_first_section_quote_line"></div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <section class="invisalign_second_section">
        <div class="container-fluid invisalign_second_section_container">
            <div class="row g-0 invisalign_second_section_row">
                <div class="col-lg-6 invisalign_second_section_left">
                    <div class="invisalign_second_section_content">
                        <h2 class="invisalign_second_section_title">
                            What Is Root Canal Treatment?                        </h2>
                        <p class="invisalign_second_section_description">
                            Root canal treatment may be recommended when the pulp inside a tooth becomes inflamed or infected because of deep decay, a crack, injury or another dental problem. The canals are carefully cleaned, shaped, filled and sealed; a final filling or crown may then be needed to protect the tooth.                        </p>
                        <a href="contact.php" class="invisalign_second_section_button">
                            <span>Book a Tooth Pain Assessment</span>
                            <i class="bi bi-arrow-right" aria-hidden="true"></i>
                        </a>
                    </div>
                </div>

                <div class="col-lg-6 invisalign_second_section_right">
                    <div class="invisalign_second_section_image_wrapper">
                        <img src="assets/img/treatments/root-canal-rec.png"
                             alt="Root canal therapy illustration"
                             class="invisalign_second_section_image"
                             loading="lazy">

                        <div class="invisalign_second_section_handwriting" aria-hidden="true">
                            <span>Clean</span>
                            <span>Seal</span>
                            <div class="invisalign_second_section_handwriting_last">
                                <span>Protect</span>
                                <i class="bi bi-heart"></i>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <section class="invisalign_third_section">
        <div class="container-fluid invisalign_third_section_container">
            <div class="invisalign_third_section_header">
                <div class="invisalign_third_section_eyebrow">KEY BENEFITS</div>
                <h2 class="invisalign_third_section_title">
                    Why Choose Root Canal Treatment?                </h2>
            </div>

            <div class="row g-4 align-items-stretch">
                <div class="col-xl-9 col-lg-8">
                    <div class="invisalign_third_section_benefits">
                                                    <div class="invisalign_third_section_card">
                                <div class="invisalign_third_section_icon">
                                    <!-- <i class="bi bi-shield-plus" aria-hidden="true"></i> -->
                                    <img src="./assets/img/icons/root-canal.png" alt="" style="height:60px; width:60px">
                                </div>
                                <div class="invisalign_third_section_card_text">
                                    Treat Infection<br>Inside the Tooth                                </div>
                            </div>
                                                    <div class="invisalign_third_section_card">
                                <div class="invisalign_third_section_icon">
                                    <!-- <i class="bi bi-heart-pulse" aria-hidden="true"></i> -->
                                    <img src="./assets/img/icons/gum-assesment.png" alt="" style="height:60px; width:60px">
                                </div>
                                <div class="invisalign_third_section_card_text">
                                    Address the<br>Source of Pain                                </div>
                            </div>
                                                    <div class="invisalign_third_section_card">
                                <div class="invisalign_third_section_icon">
                                    <!-- <i class="bi bi-emoji-smile" aria-hidden="true"></i> -->
                                    <img src="./assets/img/icons/smiling-teeth.png" alt="" style="height:60px; width:60px">
                                </div>
                                <div class="invisalign_third_section_card_text">
                                    Help Retain<br>Your Natural Tooth                                </div>
                            </div>
                                                    <div class="invisalign_third_section_card">
                                <div class="invisalign_third_section_icon">
                                    <!-- <i class="bi bi-droplet" aria-hidden="true"></i> -->
                                    <img src="./assets/img/icons/root-canal-step-3.png" alt="" style="height:60px; width:60px">
                                </div>
                                <div class="invisalign_third_section_card_text">
                                    Thorough Canal<br>Cleaning                                </div>
                            </div>
                                                    <div class="invisalign_third_section_card">
                                <div class="invisalign_third_section_icon">
                                    <!-- <i class="bi bi-lock" aria-hidden="true"></i> -->
                                    <img src="./assets/img/icons/root-canal-step-4.png" alt="" style="height:60px; width:60px">
                                </div>
                                <div class="invisalign_third_section_card_text">
                                    Filled &amp;<br>Sealed Canals                                </div>
                            </div>
                                                    <div class="invisalign_third_section_card">
                                <div class="invisalign_third_section_icon">
                                    <!-- <i class="bi bi-gem" aria-hidden="true"></i> -->
                                    <img src="./assets/img/icons/step-5.png" alt="" style="height:60px; width:60px">
                                </div>
                                <div class="invisalign_third_section_card_text">
                                    Protective Final<br>Restoration                                </div>
                            </div>
                                            </div>
                </div>

                <div class="col-xl-3 col-lg-4">
                    <div class="invisalign_third_section_cta">
                        <h3>A Healthier Smile<br>Starts Here</h3>
                        <div class="invisalign_third_section_cta_line"></div>
                        <p>Persistent toothache, swelling or sensitivity should be assessed by a dentist without delay.</p>
                        <a href="contact.php" class="invisalign_third_section_cta_button">
                            <span>Schedule a Tooth Check</span>
                            <i class="bi bi-arrow-right" aria-hidden="true"></i>
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <!-- <section class="invisalign_forth_section">
        <div class="container-fluid invisalign_forth_section_container">
            <div class="row g-0 invisalign_forth_section_row">
                <div class="col-lg-8 invisalign_forth_section_left">
                    <div class="invisalign_forth_section_content">
                        <div class="invisalign_forth_section_eyebrow">HOW IT WORKS</div>
                        <h2 class="invisalign_forth_section_title">
                            Your Root Canal Journey                        </h2>

                        <div class="invisalign_forth_section_steps">
                                                            <div class="invisalign_forth_section_step">
                                    <div class="invisalign_forth_section_step_top">
                                        <div class="invisalign_forth_section_icon">
                                            <i class="bi bi-search" aria-hidden="true"></i>
                                        </div>
                                        <span class="invisalign_forth_section_number">
                                            01                                        </span>
                                    </div>
                                    <h3>Examination</h3>
                                    <p>We assess the tooth<br>and take X-rays if needed.</p>
                                </div>
                                                            <div class="invisalign_forth_section_step">
                                    <div class="invisalign_forth_section_step_top">
                                        <div class="invisalign_forth_section_icon">
                                            <i class="bi bi-shield-check" aria-hidden="true"></i>
                                        </div>
                                        <span class="invisalign_forth_section_number">
                                            02                                        </span>
                                    </div>
                                    <h3>Numbing &amp; Isolation</h3>
                                    <p>The tooth is numbed and<br>kept clean and dry.</p>
                                </div>
                                                            <div class="invisalign_forth_section_step">
                                    <div class="invisalign_forth_section_step_top">
                                        <div class="invisalign_forth_section_icon">
                                            <i class="bi bi-droplet" aria-hidden="true"></i>
                                        </div>
                                        <span class="invisalign_forth_section_number">
                                            03                                        </span>
                                    </div>
                                    <h3>Clean &amp; Fill</h3>
                                    <p>Infected tissue is removed<br>and canals are sealed.</p>
                                </div>
                                                            <div class="invisalign_forth_section_step">
                                    <div class="invisalign_forth_section_step_top">
                                        <div class="invisalign_forth_section_icon">
                                            <i class="bi bi-gem" aria-hidden="true"></i>
                                        </div>
                                        <span class="invisalign_forth_section_number">
                                            04                                        </span>
                                    </div>
                                    <h3>Restore</h3>
                                    <p>A filling or crown helps<br>protect the treated tooth.</p>
                                </div>
                                                    </div>
                    </div>
                </div>

                <div class="col-lg-4 invisalign_forth_section_right">
                    <div class="invisalign_forth_section_image_wrapper">
                        <img src="assets/img/treatments/root-cnal-square.png"
                             alt="Root canal therapy illustration"
                             class="invisalign_forth_section_image"
                             loading="lazy">

                        <div class="invisalign_forth_section_handwriting" aria-hidden="true">
                            <span>Better</span>
                            <span>Smiles</span>
                            <span>Brighter</span>
                            <div>
                                <span>Tomorrows</span>
                                <i class="bi bi-heart"></i>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section> -->



    <section class="rootcanal_fourth_section_wrapper">
    <div class="container">
        <div class="rootcanal_fourth_section_process_card">

            <!-- SECTION HEADER -->
            <div class="rootcanal_fourth_section_header">
                <h2 class="rootcanal_fourth_section_title">
                    THE ROOT CANAL TREATMENT PROCESS
                </h2>
                <div class="rootcanal_fourth_section_divider">
                    <div class="rootcanal_fourth_section_divider_line"></div>
                    <div class="rootcanal_fourth_section_divider_dot"></div>
                    <div class="rootcanal_fourth_section_divider_line"></div>
                </div>
            </div>

            <!-- 5 STEPS FLOW -->
            <div class="rootcanal_fourth_section_steps_flow">

                <!-- STEP 1 -->
                <div class="rootcanal_fourth_section_step_item">
                    <span class="rootcanal_fourth_section_badge">01</span>
                    <div class="rootcanal_fourth_section_icon">
                        <img src="./assets/img/icons/dental-examination.png" alt="Examination and Diagnosis">
                    </div>
                    <h3 class="rootcanal_fourth_section_step_title">Examination &amp; Diagnosis</h3>
                    <p class="rootcanal_fourth_section_step_desc">
                        We examine the affected tooth and use X-rays to assess the infection and plan your treatment.
                    </p>
                </div>

                <!-- STEP 2 -->
                <div class="rootcanal_fourth_section_step_item">
                    <span class="rootcanal_fourth_section_badge">02</span>
                    <div class="rootcanal_fourth_section_icon">
                        <img src="./assets/img/icons/widom-teeth-step-2.png" alt="Local Anesthesia and Access">
                    </div>
                    <h3 class="rootcanal_fourth_section_step_title">Anesthesia &amp; Access</h3>
                    <p class="rootcanal_fourth_section_step_desc">
                        Local anesthesia is given for comfort, and a small opening is created to access the infected pulp.
                    </p>
                </div>

                <!-- STEP 3 -->
                <div class="rootcanal_fourth_section_step_item">
                    <span class="rootcanal_fourth_section_badge">03</span>
                    <div class="rootcanal_fourth_section_icon">
                        <img src="./assets/img/icons/root-canal-step-3.png" alt="Cleaning and Disinfection">
                    </div>
                    <h3 class="rootcanal_fourth_section_step_title">Cleaning &amp; Disinfection</h3>
                    <p class="rootcanal_fourth_section_step_desc">
                        The infected pulp is carefully removed, and the root canals are thoroughly cleaned and disinfected.
                    </p>
                </div>

                <!-- STEP 4 -->
                <div class="rootcanal_fourth_section_step_item">
                    <span class="rootcanal_fourth_section_badge">04</span>
                    <div class="rootcanal_fourth_section_icon">
                        <img src="./assets/img/icons/root-canal-step-4.png" alt="Root Canal Filling">
                    </div>
                    <h3 class="rootcanal_fourth_section_step_title">Canal Filling &amp; Sealing</h3>
                    <p class="rootcanal_fourth_section_step_desc">
                        The cleaned canals are filled and securely sealed to prevent bacteria from entering again.
                    </p>
                </div>

                <!-- STEP 5 -->
                <div class="rootcanal_fourth_section_step_item">
                    <span class="rootcanal_fourth_section_badge">05</span>
                    <div class="rootcanal_fourth_section_icon">
                        <img src="./assets/img/icons/step-5.png" alt="Tooth Restoration">
                    </div>
                    <h3 class="rootcanal_fourth_section_step_title">Tooth Restoration</h3>
                    <p class="rootcanal_fourth_section_step_desc">
                        A filling or custom crown is placed to restore the tooth's strength, function, and natural appearance.
                    </p>
                </div>

            </div>
        </div>
    </div>
</section>

    <section class="invisalign_fifth_section">
        <div class="container-fluid invisalign_fifth_section_container">
            <div class="row g-4 align-items-stretch">
                <div class="col-xl-3 col-lg-4">
                    <div class="invisalign_fifth_section_intro">
                        <div class="invisalign_fifth_section_eyebrow">PERSONALISED CARE</div>
                        <h2 class="invisalign_fifth_section_title">
                            Save the Tooth,<br>Restore Your Comfort                        </h2>
                        <p class="invisalign_fifth_section_description">
                            Prompt assessment can help determine whether the tooth can be treated and retained.                        </p>
                        <a href="patient-stories.php" class="invisalign_fifth_section_button">
                            <span>View Patient Stories</span>
                            <i class="bi bi-arrow-right" aria-hidden="true"></i>
                        </a>
                    </div>
                </div>

                <div class="col-xl-6 col-lg-5">
                    <div class="invisalign_fifth_section_before_after">
                        <div class="invisalign_fifth_section_photo">
                            <img src="assets/img/treatments/Root-canal-bef-img.png"
                                 alt="Root canal treatment for an infected tooth"
                                 loading="lazy">
                            <span class="invisalign_fifth_section_label before">Before</span>
                        </div>

                        <div class="invisalign_fifth_section_center_arrow" aria-hidden="true">
                            <i class="bi bi-arrow-right"></i>
                        </div>

                        <div class="invisalign_fifth_section_photo">
                            <img src="assets/img/treatments/Root-canal-aft-img.png"
                                 alt="Root canal therapy illustration"
                                 loading="lazy">
                            <span class="invisalign_fifth_section_label after">After</span>
                        </div>
                    </div>
                </div>

                <div class="col-xl-3 col-lg-3">
                    <div class="invisalign_fifth_section_testimonial">
                        <div class="invisalign_fifth_section_quote_mark" aria-hidden="true">&ldquo;</div>
                        <p class="invisalign_fifth_section_quote">
                            We explain each stage clearly and plan the final restoration needed to protect the treated tooth.                        </p>
                        <div class="invisalign_fifth_section_author">&mdash; IALIGN Care Team</div>
                        <div class="invisalign_fifth_section_stars" aria-label="Our care principles">
                            <i class="bi bi-check-circle-fill"></i>
                            <i class="bi bi-check-circle-fill"></i>
                            <i class="bi bi-check-circle-fill"></i>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
</main>

<?php include 'footer.php'; ?>

</body>
</html>
